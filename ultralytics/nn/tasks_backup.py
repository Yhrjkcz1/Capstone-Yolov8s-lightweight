...
def parse_model(d, ch, verbose=True):
    ...
    # Args
    legacy = True  # backward compatibility
    ...
    # 保持你原有的 base_modules 和 repeat_modules 不变
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        # --- 核心修改部分：安全处理通道逻辑，防止列表索引报错 ---
        if m in (ASFF_2L, ASFF_3L):
            ...
            if m in repeat_modules:
                ...
                n = 1
            # 兼容 YOLOv10/v8 特殊逻辑
            if m is C3k2 and scale in "mlx": args[3] = True
            if m is A2C2f and scale in "lx": args.extend((True, 1.2))
        elif m is AIFI:
            args = [ch[f], *args]
            ...
            # 这里的修改是预防 TypeError 的核心：如果 f 是列表且没被上面拦截，取最后一个索引
            c2 = ch[f] if not isinstance(f, list) else ch[f[-1]]
